...
DWTransformer code retrieved from:
https://github.com/H1pshurne/DW-MLSR.

Original paper:
YuXuan H.
DW-MLSR:Unsupervised Deformable Medical Image Registration Based on Dual-Window Attention and Multi-Latent Space

Inspiration source: https://github.com/Solemoon/XMorpher and https://github.com/SwinTransformer/Swin-Transformer-Semantic-Segmentation
...
def get_window_size(x_size, window_size, shift_size=None):
    ...
    for i in range(len(x_size)):
        if x_size[i] <= window_size[i] or x_size[i]%window_size[i] !=0:
            use_window_size[i] = x_size[i]
            if shift_size is not None:
                use_shift_size[i] = 0
    return tuple(use_window_size)

def Xwindow_partition(x, window_size):
    """
    Args:
        x: (B, D, H, W, C)
    ...
    return windows
def Xwindow_area_partition(x, window_size):
    """
    Args:
        x: (B, D, H, W, C)
        window_size (tuple[int]): window size
    ...
    if x.device.type == 'cuda':
        ...
        b = torch.zeros(size=(d * h * w, 27, window_size[0], window_size[1], window_size[2], C)).cpu()
...
def window_partition(x, window_size):
    ...
    return windows
def Xwindow_reverse(windows, window_size, B, D, H, W):
    """
    Args:
        windows: (B*num_windows, window_size, window_size, window_size, C)
        window_size (tuple[int]): Window size
...
class CrossTransformerBlock3D(nn.Module):
    ...
    def __init__(self, dim, num_heads, Xwindow_size=(6, 6, 6),
                 mlp_ratio=4., qkv_bias=True, qk_scale=None, drop=0., attn_drop=0., drop_path=0.,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, use_checkpoint=False):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        self.window_size = Xwindow_size
    ...
    def forward_part1(self, x, xa):
        ...
        _, Dp, Hp, Wp, _ = xa.shape

        # partition windows
        x_windows = Xwindow_partition(x, window_size)  # B*nW, Wd*Wh*Ww, C
        # x_area_windows = window_partition(xa, window_size)

        x_area_windows = Xwindow_area_partition(xa, window_size)
        # W-MSA/SW-MSA
        attn_windows = self.cross_attn(x_windows, x_area_windows)  # B*nW, Wd*Wh*Ww, C
        # merge windows
        attn_windows = attn_windows.view(-1, *(window_size + (C,)))
        x = Xwindow_reverse(attn_windows, window_size, B, Dp, Hp, Wp)  # B D' H' W' C
...
class DWSwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
        ...
        self.attn=CrossWindowAttention3D( dim, window_size=self.window_size, num_heads=num_heads,
                                          qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop)
    ...
    def forward(self, x):
        ...
        x = self.norm1(x)
        x = x.view(B, H, W, T, C)
        window_size = get_window_size((H, W, T), self.window_size)
        # pad feature maps to multiples of window size
        pad_l = pad_t = pad_f = 0
        pad_r = (window_size[0] - H % window_size[0]) % window_size[0]
        pad_b = (window_size[1] - W % window_size[1]) % window_size[1]
        pad_h = (window_size[2] - T % window_size[2]) % window_size[2]
        ...
        # partition windows
        x_windows = Xwindow_partition(shifted_x, window_size)  # nW*B, window_size, window_size, window_size, C
        x_area_windows=Xwindow_area_partition(shifted_x,window_size)
        x_windows = x_windows.view(-1, window_size[0] * window_size[1] * window_size[2], C)  # nW*B, window_size*window_size*window_size, C

        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows, x_area_windows)  # B*nW, Wd*Wh*Ww, C

        # merge windows
        attn_windows = attn_windows.view(-1, window_size[0], window_size[1],window_size[2], C)
        shifted_x = window_reverse(attn_windows, window_size, Hp, Wp, Tp)  # B H' W' L' C
...
class DWSwinTransformer(nn.Module):
    r""" Swin Transformer
    ...
    def train(self, mode=True):
        ...
        super(DWSwinTransformer, self).train(mode)
        self._freeze_stages()
...
class DWTransMorphforReg(nn.Module):
    def __init__(self, config):
        '''
        TransMorph Model
        '''
        super(DWTransMorphforReg, self).__init__()
        if_convskip = config.if_convskip
        self.if_convskip = if_convskip
        if_transskip = config.if_transskip
        self.if_transskip = if_transskip
        embed_dim = config.embed_dim
        self.transformer = DWSwinTransformer(patch_size=config.patch_size,
...
class DWTransMorphforSeg(nn.Module):
    def __init__(self, config):
        '''
        DWTransMorph Model
        '''
        super(DWTransMorphforSeg, self).__init__()
        ...
        self.up1 = DecoderBlock(embed_dim*4, embed_dim*2, skip_channels=embed_dim*2 if self.if_transskip else 0, use_batchnorm=False)  # 384, 20, 20, 64
        self.up2 = DecoderBlock(embed_dim*2, embed_dim, skip_channels=embed_dim if self.if_transskip else 0, use_batchnorm=False)  # 384, 40, 40, 64
        self.up3 = DecoderBlock(embed_dim, embed_dim//2, skip_channels=embed_dim//2 if self.if_convskip else 0, use_batchnorm=False)  # 384, 80, 80, 128
        self.up4 = DecoderBlock(embed_dim//2, config.reg_head_chan, skip_channels=config.reg_head_chan if self.if_convskip else 0, use_batchnorm=False)  # 384, 160, 160, 256
...
CONFIGS = {
    'DWTransMorphforReg': configs.get_DWTransMorphforReg_config(),
    'DWTransMorphforSeg': configs.get_DWTransMorphforSeg_config(),
}
